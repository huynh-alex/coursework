...
public class Tableau {
	...
	private SpaceShip getLeftMostEnemy() {
		...
		int xOut = enemyFleet[0].length;
		int yOut = enemyFleet.length;
		...
		if (xOut < enemyFleet[0].length && yOut < enemyFleet.length) {
			return enemyFleet[yOut][xOut];
		} else {
			return null;
		}
	...
	public void moveEnemyFleet() {
		...
		} else if (getLeftMostEnemy().getPosition().getX() <= 0) {
			fleetMovePositiveX = true;
			translateEnemyFleet(0, FLEET_MOVE_Y);
		}
